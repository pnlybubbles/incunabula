...
module.exports = (state, emitter) => {
  ...
  emitter.on(keys.file.hotReload, () => {
    ...
    console.log(state.file.list, state.file.current)
  })

  emitter.on(keys.file.open, () => {
    fileio.open((err, docs) => {
      if (err) {
        console.error(err)
        return
      }
      docs.forEach((doc) => {
        ...
        if (item) {
          ...
          if (item.lastLoad !== doc.content) {
            ...
            console.info(`File (${doc.path}) is loaded, but content is not matched. It probably the file has been updated with the other editors.`)
          } else {
